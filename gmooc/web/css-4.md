...
- 상속은 부모 요소의 속성을 자식 요소가 물려받는 것을 의미합니다. 
- 상속을 이용하면 코드의 중복성을 줄여주기 때문에 생산성을 높일 수 있습니다. 
- 코드 수정시, 부모 요소의 수정으로 모든 자식 요소에도 같은 효과를 가져오기 때문에 유지 보수의 시간을 최소화시켜줍니다.
- \<body>태그에 폰트를 노란색으로 한다면 \<body>태그의 자식요소는 그 속성을 물려받아 사용할 수 있습니다.
...
- \<body>태그의 모든 하위 항목은 \<div>, \<img>, \<h1>

#### 2) 선택자A > 선택자B 
- 선택자A의 `자손`인 선택자B 선택.
- 지정된 요소의 `직접 하위 요소`인 모든 요소를 선택합니다.
- \<div>태그의 직접 하위 항목은 \<img>, \<h1>

#### 3) 선택자A + 선택자B
- 가장 가까운 형제 요소에 속성 적용.
- 형제 요소는 같은 부모를 가져야합니다.
- 첫 번째 \<img>태그의 인접 형제 요소는 \<img>

#### 4) 선택자A ~ 선택자B
- 지정된 요소의 형제인 모든 요소를 선택합니다.
- 첫 번째 \<img>태그의 모든 형제 요소는 \<img>, \<h1>
...
- 가상 클래스는 이름 앞에 `콜론(:)`을 붙여 표시합니다.
- 가상 요소는 클래스 이름 앞에 `콜론 두개(::)`를 붙여 표시합니다.
- 사용자 동작에 반응하는 가상클래스 (link, visited, hover, active, focus)
- UI 요소 상태에 따른 가상클래스 (enable, disabled, checked)
